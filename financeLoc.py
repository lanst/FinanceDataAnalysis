...
def main():
    ...
    for CIK in cikList:
        ...
        myResults = {}
        allFilingsReferences, SIC, state = getFilingsLinks(
...
#        print(allFilingsReferences)
    ...
    dataColumnHeadersK = ["CIK", "SIC", "Report Date", "State", "MD&A subsection", "Positive words", "Negative words", "Difference",
                          "Total # words"]
    dataColumnHeadersQ= ["CIK", "End of Quarter Date", "SIC", "Report Date", "State", "MD&A subsection", "Positive words", "Negative words", "Difference", "Total # words"]
...
def getFilingsLinks(CIK):
    ...
    while True:  # while the newly generated webpage still contains new links
        ...
        for title in tdSoup:  # dump file name, file date, and link to file into a dictionary
            ...
            indexCounter += 1
        indexCounter = 0;
        pSoup = soup.find_all("p", class_ = "identInfo")
        #print ("PRINTING OUT PSOUP")
        indexCounter = 0
        #print (pSoup)
        for title in pSoup:
            if "State of Inc." in title.text in title.text:
                newLinkFlag = True
                locationMessy = str(pSoup[indexCounter]).split('|')[1]
                location = locationMessy[24:26]
            else:
                location = "No State"
            indexCounter+=1
        #print (location)
    ...
    return filingsData, SIC, location  # returning dict of links and SIC here to reduce the number of requests the are needed per file/CIK
...
def wordFrequencyAnalysis(texs, CIK):
    ####################################################################################
